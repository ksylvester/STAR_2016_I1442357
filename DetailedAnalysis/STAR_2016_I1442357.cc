...
/// ***************************************************************************************************
/// This is an analysis template created by JN -- changes have been made by KS to attempt an analysis of a specific paper
...
namespace Rivet
{
	...
	//K: I changed this from STAR_2014_I1234567 to STAR_2016_I1442357
	...
	private:
		// number of centrality bins that you need to worry about, this case two, 10-30, 50-80
		//K: I changed this from 2 to 1
		const static int N_CENT_TYPES = 1;
		//K: I changed this from {{10,30},{50,80}} to {{0,12}}
		const static int CENT_TYPE_EDGES[N_CENT_TYPES][1] = {{0,12}}; // 0-12

		// histograms; Yours will almost certainly be different
		// delta phi, split by centrality
		Scatter2DPtr _h1dPhi[N_CENT_TYPES];
	...
	public:
		...
		/// Every histogram you will use must be booked, and every projection you will use must be declared.
		void init()
		{
			//**** Select centrality method ****
			// The 50 here is the number of events used JUST to determine future events' centrality.
			// These events will show a centrality of -1.0, which is not physical.
			// You can adjust this number for testing, but never have it lower than the number of events you are testing against.
			...
			// Many of you will have "trigger" particles. This is code to trigger off of a specific type or particle in a certain range.
			// There will ALWAYS be an "abseta" cut on all of your particles. It may differ from your trigger and associated particles.
			// This example uses Pi0 and gamma of different energies as triggers.
			...
			//K: Changed from.... Cuts::abseta < 1.0 && ((Cuts::pid == pidPI0 && Cuts::pt > 13.0 * GeV && Cuts::pt < 20.0 * GeV)
			//                    || (Cuts::pid == pidGAMMA && Cuts::pt > 8.0 * GeV && Cuts::pt < 20.0 * GeV))
			//       To.... Cuts::abseta < 0.9 && ((Cuts::pid == pidPI0 && Cuts::pt > 12.0 * GeV && Cuts::pt < 20.0 * GeV)
			...
			// But that is based on the specific trigger. So that check will be done later inside a loop in analysis
			//K: Changed from.... Cuts::abseta < 1.0 && Cuts::pt > 1.2 * GeV && Cuts::pt < 20.0 * GeV;
			//       To.... Cuts::abseta < 1.0 && Cuts::pt > 1.2 * GeV && Cuts::pt < 3 * GeV;
			...
			declare(cfs, "partAssoc");
				...
				for (int i = 0; i < N_CENT_TYPES; ++i) {
					// This creates a histogram with the name "d01-x01-yNN"
					// where NN = i and uses the reference data from the .yoda file associated with this analysis
					_h1dPhi[i] = bookScatter2D(i,1,1);
				}

			//**** Initialize counters ****
			// Set number of events in counters to zero
			// These are used later for normalizing histograms in finalize
			for (int i = 0; i < N_CENT_TYPES; ++i) nTrigger[i] = 0;
		}

		/// Per event calculations
		/// Do your per event calculations and fill your histograms here
		void analyze(const Event& event)
		{
			...
			// In the background RIVET is handling the the declared projections but you need to access them inside analyze to use tCFShem
			...
			// Its a simplification of a for loop where you would have had to find the number of entries and loop over them.
			...
			foreach (const Particle& partTrigger, tracksTrigger) {
				...
				foreach (const Particle& partAssoc, tracksAssoc) {
					...
					if (partAssoc.pt() < partTrigger.pt()) {
						...
						// In this fake paper plots run from 0 - 2 pi so make sure deltaPhi falls in this range
						...
						// K: My deltaPhi is also 0-2PI for this paper (Full Azimuthal Acceptance)
						...
						// With event weighting factored in, that number would be variable and nTrigger would need to factor in weighting.

						//K: I was told not to worry about changing the weight at this time.
		...
		// Here I need to normalize my histogram by dividing by the number of triggers.
		void finalize()
		{
			for (int i = 0; i < N_CENT_TYPES; ++i){
				...
				// Histo1Dptr are just pointers to YODA objects, and their documentation is in the YODA docs.
